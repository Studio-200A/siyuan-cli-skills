...
- **CLI execution semantics:** the installed CLI's live help determines current command paths, flags, and input modes. Selected behaviors that live help cannot reveal or describes inaccurately were checked against SiYuan CLI 3.7.3, the official [`kernel/cli/cmd`](https://github.com/siyuan-note/siyuan/tree/master/kernel/cli/cmd) implementation, and observed output, then recorded as version-qualified caveats.
